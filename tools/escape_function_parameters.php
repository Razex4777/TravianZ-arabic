...
';
	...
	// uncomment the following lines to make this work and provide output:
	...
	//	$re = '/(\$[^ \t=,]+)[ \t]*=[ \t,]*[^,)]*/';
	//	$subst = '\\1';
	//	$matches[2][$index] = preg_replace($re, $subst, $matches[2][$index]);

	//	$subject = str_replace($value, $value . "\n" . '        list(' . $matches[2][$index] . ') = $this->escape_input(' . $matches[2][$index] . ');' . "\n", $subject);
	//}
