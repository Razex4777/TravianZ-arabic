...
for($i=1;$i<=50;$i++) {
	if($troops['u'.$i] > 0) {
		...
		echo "<td class=\"num\">".number_format($troops['u'.$i])."</td><td class=\"un\">".$technology->getUnitName($i)."</td></tr>";
		$TroopsPresent = True;
	}
}
if($troops['hero'] > 0) {
		echo "<tr><td class=\"ico\"><a href=\"build.php?id=39\"><img class=\"unit uhero\" src=\"img/x.gif\" alt=\"".HERO."\" title=\"".HERO."\" /></a></td>";
		echo "<td class=\"num\">".number_format($troops['hero'])."</td><td class=\"un\">".HERO."</td></tr>";
		$TroopsPresent = True;
}
