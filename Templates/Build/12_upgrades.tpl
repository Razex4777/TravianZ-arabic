...
<a href=\"#\" onClick=\"return Popup(".$i.",1);\">".$technology->getUnitName($i)."</a> (<?php echo LEVEL; ?> ".$abdata['b'.$j];
				$ups = 0;
				if($totalUps > 0){
		        ...
		        if($abdata['b'.$j] == 20) {
				    ...
				    echo "<td class=\"act\"><div class=\"none\">".UPGRADE_BLACKSMITH."</div></td>";
				}
				...
				if ($abCurLevel >= 20) {
				    ...
				    if ($session->gold >= $goldNeeded) {
				        echo "<a href=\"build.php?id=$id&amp;upgradeABMax=1&amp;abType=b&amp;abUnit=$j\">".((defined('LANG') && LANG === 'ar') ? 'تطوير إلى المستوى 20' : 'Upgrade to Level 20')."</a><br/>";
				    } else {
